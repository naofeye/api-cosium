"""Service d'export PDF : dashboard, balance clients et rapport mensuel."""
...
def export_monthly_report_pdf(
    ...
    # Reusable styles
    title_style = ParagraphStyle(
        "MonthlyTitle", parent=styles["Heading1"],
        fontSize=18, alignment=1, spaceAfter=6,
    )
    date_style = ParagraphStyle(
        "MonthlyDate", parent=styles["Normal"],
        fontSize=10, alignment=1, spaceAfter=20, textColor=rl_colors.grey,
    )
    section_style = ParagraphStyle(
        "MonthlySection", parent=styles["Heading2"],
    ...
    # ── Header ──
    elements.append(Paragraph(
        f"Rapport mensuel — {month_name} {year} — OptiFlow",
        title_style,
    ))
    generated = datetime.now(UTC).strftime("%d/%m/%Y %H:%M")
    elements.append(Paragraph(f"Genere le {generated}", date_style))

    # ── 1. KPIs du mois ──
    elements.append(Paragraph("1. KPIs du mois", section_style))
    ...
    kpi_data = [
        ["Indicateur", "Valeur"],
        ["CA Total", _fmt_money(ca_total)],
        ["Encaisse", _fmt_money(encaisse)],
        ["Impayes", _fmt_money(impayes)],
    ...
    # ── 2. Activite du mois ──
    elements.append(Paragraph("2. Activite du mois", section_style))
    ...
    elements.append(Paragraph("3. Top 10 clients par CA", section_style))
    ...
    for i, row in enumerate(top_rows, 1):
        ...
        top_data.append([str(i), name, _fmt_money(float(row.ca))])
    ...
    # ── 4. Balance agee (impayes par tranche) ──
    elements.append(Paragraph("4. Balance agee (impayes)", section_style))
    ...
    for tranche, amount in buckets.items():
        aging_data.append([tranche, _fmt_money(round(amount, 2))])
        total_aging += amount
    aging_data.append(["TOTAL", _fmt_money(round(total_aging, 2))])
    ...
    elements.append(Spacer(1, 10 * mm))
    footer_style = ParagraphStyle(
        "MonthlyFooter", parent=styles["Italic"],
