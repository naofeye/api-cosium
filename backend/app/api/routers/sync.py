...
@router.post("/customers", response_model=SyncResultResponse)
def sync_customers(
    db: Session = Depends(get_db),
    tenant_ctx: TenantContext = Depends(require_tenant_role("admin", "manager")),
) -> SyncResultResponse:
...
@router.post("/invoices", response_model=SyncResultResponse)
def sync_invoices(
    db: Session = Depends(get_db),
    tenant_ctx: TenantContext = Depends(require_tenant_role("admin", "manager")),
) -> SyncResultResponse:
...
@router.post("/products", response_model=SyncResultResponse)
def sync_products(
    db: Session = Depends(get_db),
    tenant_ctx: TenantContext = Depends(require_tenant_role("admin", "manager")),
) -> SyncResultResponse:
...
@router.get("/erp-types", response_model=list[ERPTypeItem])
def list_erp_types(
