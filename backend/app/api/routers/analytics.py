...
@router.get("/aging", response_model=AgingBalance)
def get_aging(
    db: Session = Depends(get_db),
    tenant_ctx: TenantContext = Depends(get_tenant_context),
) -> AgingBalance:
...
@router.get("/payers", response_model=PayerPerformance)
def get_payers(
    db: Session = Depends(get_db),
    tenant_ctx: TenantContext = Depends(get_tenant_context),
) -> PayerPerformance:
...
@router.get("/commercial", response_model=CommercialKPIs)
def get_commercial(
    db: Session = Depends(get_db),
    tenant_ctx: TenantContext = Depends(get_tenant_context),
) -> CommercialKPIs:
...
@router.get("/marketing", response_model=MarketingKPIs)
def get_marketing(
    db: Session = Depends(get_db),
    tenant_ctx: TenantContext = Depends(get_tenant_context),
) -> MarketingKPIs:
...
@router.get("/dashboard", response_model=DashboardFull)
def get_dashboard(
    date_from: datetime | None = Query(None),
    date_to: datetime | None = Query(None),
    db: Session = Depends(get_db),
