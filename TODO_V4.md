...
- [x] `models/payment.py` — Changer `Mapped[float]` en `Mapped[Decimal]` pour `amount_due`, `amount_paid` (colonnes Numeric(10,2))
- [x] `models/devis.py` — Mapped[Decimal] pour `montant_ht`, `tva`, `montant_ttc`, `part_secu`, `part_mutuelle`, `reste_a_charge`
- [x] `models/facture.py` — Idem pour tous les montants
- [x] `models/devis.py` (DevisLigne) — Idem pour `prix_unitaire_ht`, `montant_ht`, `montant_ttc`
- [x] `models/pec.py` (PecRequest) — Idem pour `montant_demande`, `montant_accorde`
- [x] `models/bank_transaction.py` — Idem pour `montant`
- [x] Ajouter `from decimal import Decimal` dans chaque fichier modifie
...
- [x] `api/routers/clients.py` — Lock `acquire_lock(f"merge:{tenant_id}:{keep_id}:{merge_id}", ttl=60)` avant d'appeler `merge_clients()`
- [x] Ajouter `release_lock()` dans un `finally` block

### 2.2 Sync error propagation [ELEVE]
- [x] `api/routers/sync.py:234-280` — Quand `has_errors=True`, retourner HTTP 207 (Multi-Status) au lieu de 200
- [x] Ou alternativement, ajouter un champ `"success": false` explicite dans la reponse
...
- [x] `tasks/reminder_tasks.py` — Subject sanitise le subject email avec `re.sub(r'[\r\n\t]', ' ', subject)` avant envoi

### 3.2 Repo get_by_id retourne les clients supprimes [ELEVE]
- [x] `repositories/client_repo.py` — `get_by_id()` doit filtrer `deleted_at IS NULL` par defaut (comme `get_by_id_active`)
- [x] Renommer l'ancien `get_by_id()` en `get_by_id_including_deleted()` pour les cas ou c'est necessaire (restore, admin)
- [x] Mettre a jour tous les appelants qui utilisaient `get_by_id()` (12 services)
...
- [x] `components/ui/InlineEdit.tsx` — Le `setTimeout` dans `onBlur` n'est pas nettoye si le composant se demonte → ajouter cleanup useEffect

### 4.2 Token refresh timeout race [ELEVE]
- [x] `lib/api.ts` — Le timeout AbortController du premier appel n'est pas annule lors du retry apres refresh → clearTimeout avant retry

### 4.3 SSE reconnexion hang [ELEVE]
- [x] `lib/sse.ts` — La reconnexion peut bloquer si `onerror` fire plusieurs fois rapidement → verifier que eventSource est bien null avant reconnect

### 4.4 Popstate navigation trap [ELEVE]
- [x] `hooks/useUnsavedChangesWarning.ts` — `e.preventDefault()` est un no-op sur popstate → corriger le back button loop

### 4.5 SWR deduping masque les mutations [MOYEN]
- [x] `lib/swr.tsx` — Reduire `dedupingInterval` de 5000ms a 2000ms pour que les creations apparaissent plus vite

### 4.6 useTransition pending non reset [MOYEN]
- [x] `clients/page.tsx` — Reset showDuplicates de doublons echoue, `dupesPending` reste `true` indefiniment → reset dans le catch

### 4.7 Keyboard shortcuts force reload [MOYEN]
- [x] `lib/shortcuts.ts` — `window.location.href` navigations → remplacer par `router.push()` pour eviter le rechargement complet
...
- [x] `backend/app/db/session.py:9` — `CELERY_WORKER` utilise via `os.environ` au lieu de Settings → migrer dans config.py
- [x] `backend/app/main.py:180` — `SEED_ON_STARTUP` utilise via `os.environ` → migrer dans config.py
- [x] `.env.example` — Ajoute `CELERY_WORKER=` et `SEED_ON_STARTUP=true` avec commentaires

### 5.2 Transaction per-document dans cosium_document_sync [MOYEN]
- [x] `services/cosium_document_sync.py:140-146` — Deplacer `db.commit()` hors de la boucle (batched commit)
- [x] Ajouter compensation si MinIO upload echoue apres db.add()
