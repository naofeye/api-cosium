...
- [ ] **Smoke tests services critiques** : partiellement fait — `analytics_cosium_service` + `client_merge_service` + `pec_consolidation_service` couverts (8 tests dans `test_services_smoke.py`, happy path + edge case + isolation tenant). Reste : `marketing_service`, `consolidation_service`, `client_360_finance`, `client_360_documents`, `batch_processing_service`, `erp_sync_invoices`, `erp_sync_payments`.
