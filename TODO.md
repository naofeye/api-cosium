...
- [x] ~~Tests E2E Playwright frontend~~ : setup `@playwright/test` + `otplib@12` dans `apps/web/`. 3 specs / 10 tests dans `tests/e2e/` (login UI+API, MFA flow TOTP dérivé, clients CRUD + logout). Helpers `helpers.ts`. Workflow CI `.github/workflows/e2e.yml` avec **nginx reverse proxy** (port 80 → `/api/*` vers :8000 + `/` vers :3000) pour same-origin cookies httpOnly. Trigger auto push master + PR + workflow_dispatch. `NEXT_PUBLIC_API_BASE_URL=/api/v1` relatif. Scripts npm `test:e2e` + `test:e2e:ui`.
