...
- [x] ~~**ESLint strict**~~ _(fait : `no-explicit-any` deja a 0 occurrence, `exhaustive-deps` 6 warnings corriges (useMemo sur les `?? []` defaults, capture timersRef avant cleanup, `toast` deps). Les 2 regles passees `"warn"` -> `"error"` dans `eslint.config.mjs`. CI lint vert.)_
