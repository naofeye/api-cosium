...
- [x] ~~Tests E2E Playwright frontend~~ : setup `@playwright/test` + `otplib@12` dans `apps/web/`. 3 specs / 10 tests dans `tests/e2e/` : `login.spec.ts` (UI + API, happy path + erreur + lien forgot-password), `mfa.spec.ts` (enroll → secret → TOTP dérivé → login API/UI avec code, cleanup auto), `clients.spec.ts` (login → créer via API → visible dans liste UI → delete cleanup + logout redirige). Helpers partagés `helpers.ts`. Workflow CI `.github/workflows/e2e.yml` (postgres service, API uvicorn + Next build/start, Playwright chromium, upload report artifacts). Scripts npm `test:e2e` + `test:e2e:ui`. README dans `tests/e2e/`.
