"""Service factures : create_from_devis + queries.

Logique avoir extraite dans `_factures/_avoir.py`, envoi email dans
`_factures/_email.py`. Les fonctions `create_avoir` + `send_facture_email`
sont re-exportees ici pour preserver l'import classique
`from app.services import facture_service; facture_service.create_avoir(...)`.
"""
...
# Re-exports pour compat (tests historiques + appelants externes)
from app.services._factures._avoir import create_avoir  # noqa: F401
from app.services._factures._email import send_facture_email  # noqa: F401
...
def get_facture_detail(db: Session, tenant_id: int, facture_id: int) -> FactureDetail:
    ...
    return FactureDetail(
        **data,
        lignes=[FactureLigneResponse.model_validate(line) for line in lignes],
    )
